...
class UserReviewJob implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($userReview, $type = 1)
    {
    ...
    {
        if ($user = $userService->getById($this->userReview->{MainContract::CUSTOMER_ID})) {
            if ($notificationTenant = $notificationTenantService->create([
                ...
                MainContract::USER_REVIEW_ID    =>  $this->userReview->{MainContract::ID},
                MainContract::VIEW  =>  0,
            ...
            event(new UserReviewTenantEvent(new UserReviewResource($this->userReview)));
        }
    }
}
